...
def resize(x, dim=None):
    if dim is not None:
        dim = x.get_shape().as_list()
    x = tf.image.resize(x, dim, method='nearest')
...
def contextual_attention(f, b, mask=None, ksize=3, stride=1, rate=1,
    ...
    f = resize(f, dim=[int(raw_int_fs[1] / (1. / rate)), int(raw_int_fs[2] / (1. / rate))])
    b = resize(b, dim=[int(raw_int_bs[1] / rate), int(raw_int_bs[2] / rate)])
    if mask is not None:
        mask = resize(mask)
